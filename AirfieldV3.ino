...
// Short Range Definitions
#define ULTRASONIC_LOW_THRESH   40  // Units: mm
#define ULTRASONIC_HIGH_THRESH  300 // Units: mm
...
// Function get_ultrasonic_distance
// Resturns distance in mm, or 0xFFFF if invalid
uint16_t get_ultrasonic_distance(byte high, byte low, byte sum) {
      byte true_sum = 0xFF + high + low;
      bool valid_measurement = (true_sum == sum);
...
void shortRangeStage() {
  ...
  while(millis() - short_active_time < reCalTime2) {
    ...
    if (header == 0xFF) {
      ...
      uint16_t distance = get_ultrasonic_distance(data_h, data_l, checksum);
      bool valid_data = (distance != 0xFFFF);
      if (valid_data) {
        Serial.print("Distance: ");
        Serial.print(distance);
        Serial.println("mm");  

        if (distance >= ULTRASONIC_LOW_THRESH && distance <= ULTRASONIC_HIGH_THRESH) {
            // Car detected! Fire!
