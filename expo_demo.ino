/********************************************
R.O.C.K. Expo Demo Program
Version: 2.0
Author: Jack Foster and Branson Camp
Date: 4/24/2024
Description: Serial interface to activate
the various sensors during the Expo demo.
*********************************************/
...
// Time Delays
#define CAPACITOR_CHARGE_TIME   15500
#define JETSON_BOOTUP_TIME      30000

 // How much time to wait for Jetson to recognize car
#define JETSON_WAIT_TIME        70000

 // How much time to wait for Short Range to recognize car
#define SHORT_RANGE_WAIT_TIME   30000
...
uint32_t reCalTime1 = JETSON_WAIT_TIME;
uint32_t reCalTime2 = SHORT_RANGE_WAIT_TIME;
...
void longRangeStage() {
  ...
  while (!receivedSignal) {
    Serial.println("Long range waiting for 'signal' (Type 'j')...");
    ...
    delay(10); // To make UI more readable
  }

  // Signal has been received. Send signal to Jetson.
...
void mediumRangeStage() {
  ...
  while(millis() - timeOfMaybe < reCalTime1){
    ...
    Serial.print("Jeston looking for car... ('s' to override) ");
    ...
    if (digitalRead(PIN_SHORT_RANGE_ACTIVATION) == LOW || serial_override) {
      Serial.println("Jetson has recognized a vehicle.");
      Serial.println("Short range sensor engaged.");
      debugSend('E');
      state = SHORT_RANGE;
      return;
    }
...
void shortRangeStage() {
  ...
  while(millis() - short_active_time < reCalTime2){
    ...
    if (Serial.available() > 0) {
      byte value = Serial.read();
      if (value == 'f') {
        // Serial Override Fire Solenoid
        Serial.println("Fire Solenoid!");
        digitalWrite(PIN_FIRE, HIGH);
        delay(100);
        digitalWrite(PIN_FIRE, LOW);

        //delay(reCalTime2 - (millis() - short_active_time)); // Do we need this for demo?
        debugSend('F');
        state = RESET_SYSTEM;
        return;
      }
    ...
    Serial.print("Short range scanning for car... ('f' to override) ");
    Serial.print("Quitting in ");
    Serial.print((reCalTime2 - (millis() - short_active_time)) / 1000);
    Serial.println(" seconds.");
    do {
      for (int i = 0; i < 4; i++) {
...
void reset(){
  Serial.println("Waiting for Jetson shutoff... (7s)");
  delay(7000);
  chargeCapacitor();
  state = LONG_RANGE;
}
...
void setup() {
  ...
  Serial.println("Waiting for Jetson bootup... (30s)");
  delay(JETSON_BOOTUP_TIME);
